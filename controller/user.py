...
from schemas.user import (
    ...
    card_interface,
    unproduct_interface,
    edit_address_interface,
    order_confirmation_interface,
    user_shop_interface,
    product_id_interface,
)
...
async def user_login(
    log_data: login_interface, request: Request, user_agent: str = Header(None)
):
    Type = log_data.type  # 获取类型
    user_information = user_model.get_user_by_usernametype(
        log_data.username, log_data.type
    )
    if user_information is None:  # 为空说明用户名不存在
        ...
        if HashPassword == user_information.password:
            ...
            session = session_interface(
                user_id=int(user_information.id),
                ip=request.client.host,
                func_type=0,
                token=token,
                user_agent=user_agent,
                exp_dt=get_time_now("days", 14),
            )
...
async def user_edit(
    ...
    if Phone is not None:
        valid_phone = user_model.get_user_by_phone(
            Phone, Token
        )  # 判断用户修改后的手机号是否被其他用户使用
        if valid_phone is not None:  # 不为空说明手机号已经存在
            return {
                "message": "该手机号已经被使用，请重新输入",
                "data": False,
                "code": 1,
            }

    if Id_card_number is not None:
        valid_id = user_model.get_user_by_id(
            Id_card_number, Token
        )  # 判断用户修改后的身份证号是否被其他用户使用
        if valid_id is not None:  # 不为空说明身份证号已经存在
            return {
                "message": "该身份证号已经被使用，请重新输入",
                "data": False,
                "code": 1,
            }
    ...
    if Photo is not None:
        ...
        if Photo.content_type.startswith("application"):
            user_model.save_upload_file(
                Photo, f"F:\\fastApiProject\static\\user_photo/{count}.jpg"
            )  # 保存文件到指定位置
...
async def user_shop(request: Request):
    ...
    for item in Shop:
        ...
        for order in order_list:
            product = order_interface()
            product_infor = user_model.get_product_by_product_id(
                order.product_id
            )  # 根据商品id获取商品信息
...
async def user_close_shop(request: Request):
    ...
    return {
        "message": "返回成功",
        "data": {
            "Audit_shop": Audit_shop,
            "Close_Shop": Close_shop,
            "Reapply_shop": Reapply_shop,
        },
        "code": 0,
    }
...
async def user_look_shop(request: Request, log_data: shop_id_interface):
    ...
    return {
        "message": "查看成功",
        "data": {"Shop": shop, "Product": Products},
        "code": 0,
    }
