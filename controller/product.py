...
from schemas.product import product_add_interface, ProductRequest, ProductSearch
    ...
    big_picture = [
        {
            "id": product.id,
            "name": product.name,
            "url": "/static/bigpicture/" + str(product.id) + ".png",
            "price": str(product.price),
        }
        for product in big_picture_data
    ]

    return {"big_pictures": big_picture, "recommends": recommendation, "code": 0}
...
@standard_response
async def shopkeeper_add_product(
    request: Request,
...
async def comment_add(request: Request, temp_comment: comment_add):
    ...
    data = {"content": temp_comment.review}
...
async def refund(request: Request, log_data: pro_refund):
    ...
    product_model.refund_deal(
        temp_order.product_id, temp_order.quantity, temp_order.amount, log_data.order_id
    )
    return {"message": "success", "data": False, "code": 0}
...
async def dialog(request: Request, log_data: dialog_add):
    ...
    product_model.add_dialog(
        send_id=User.id, receive_id=receive_id, dialog=log_data.dialog
    )
    return "success"
