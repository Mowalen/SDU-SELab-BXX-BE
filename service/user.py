...
class UserModel(dbSession, dbSessionread):
    def get_user_by_usernametype(
        self, username, type
    ):  # 根据username和type查询user的基本信息
    ...
    def get_finished_order_by_id(self, uid, pid):
        with self.get_db() as session:
            ID = (
                session.query(Order)
                .filter(
                    Order.user_id == uid, Order.product_id == pid, Order.status == 4
                )
                .all()
            )
            session.commit()
    ...
    def get_username_by_id(self, id):
        with self.get_db_read() as session:
            user = (
                session.query(User).filter(User.id == id, User.has_delete == 0).first()
            )
            session.commit()
    ...
    def get_user_by_username(self, username):  # 只根据username进行查询
        with self.get_db_read() as session:
            user = (
                session.query(User)
                .filter(User.username == username, User.has_delete == 0)
                .first()
            )
            session.commit()
            return user
    ...
    def add_user(
        self, obj: user_add_interface
    ):  # 管理员添加一个用户(在user表中添加一个用户)
    ...
    def edit_user(self, obj: user_edit_interface):  # 用户修改个人信息
        ...
        with self.get_db() as session:
            if Address != None:
                session.query(User).filter(ID.user_id == User.id).update(
                    {"address": Address}
                )
                session.commit()
            if Phone != None:
    ...
    def get_user_by_phone(
        ...
        with self.get_db_read() as session:
            phone = (
                session.query(User)
                .filter(User.phone_number == phone_number, User.id != ID.user_id)
                .first()
            )
            session.commit()
            return phone
    ...
    def get_user_by_id(self, id_card, Token):  # 获取用户的身份证号，防止重复被使用
        ...
        with self.get_db_read() as session:
            id = (
                session.query(User)
                .filter(User.id_card_number == id_card, User.id != ID.user_id)
                .first()
            )
            session.commit()
            return id
    ...
    def get_order_by_shop_id(self, shop_id):  # 根据商家所有商品获取他的所有订单
        ...
        with self.get_db() as session:
            product = (
                session.query(Order).filter(Order.product_id.in_(product_id_list)).all()
            )
            session.commit()
            return product
    ...
    def save_upload_file(
        self, upload_file: UploadFile, destination: str
    ):  # 用户上传自己头像（商家也通过该函数上传店铺图片）
        # print(destination)
    ...
    def get_check_shop(self, user_id: int):
        with self.get_db() as session:
            shop = (
                session.query(Shop)
                .filter(Shop.user_id == user_id, Shop.status != 1)
                .all()
            )
            session.commit()
            return shop
    ...
    def look_unproduct(self, id: int):
        with self.get_db() as session:
            order = (
                session.query(Product)
                .filter(Product.shop_id == id, Product.status == 0)
                .all()
            )
            session.commit()
            return order
    ...
    def look_unproducts(self, id: int):  # 商家查看未通过商品
        with self.get_db() as session:
            order = (
                session.query(Product)
                .filter(Product.shop_id == id, Product.status == 2)
                .all()
            )
            session.commit()
            return order
    ...
    def update_now_user(self, tempid: int):
        with self.get_db() as session:
            session.query(User).filter(
                and_(User.id == tempid, User.identity_type == 4)
            ).update({"identity_type": 0})
            session.query(User).filter(
                and_(User.id == tempid, User.identity_type == 5)
            ).update({"identity_type": 1})
            session.commit()
...
class SessionModel(dbSession, dbSessionread):
    ...
    def delete_session_by_token(self, token: str):  # 根据token删除一个session
        with self.get_db() as session:
            session.query(Session).filter(Session.token == token).update(
                {"has_delete": 1}
            )
            session.commit()
            return "ok"
