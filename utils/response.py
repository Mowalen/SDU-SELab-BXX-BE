...
from schemas.page import page, pageResult
...
def makePageResult(pg: page, tn: int, data: List):  # 处理分页数据
    return pageResult(
        totalNum=tn, totalPage=(tn + pg.pageSize - 1) // pg.pageSize, rows=data
    ).model_dump()
    ...
    async def decorator(*args, **kwargs):
        ...
        if "token_header" in result:
            if result["token_header"] == "-1":
                ...
                response.set_cookie(
                    key="TOKEN", value=str(result["token_header"]), max_age=24 * 3600
                )  # 添加cookie
        if "token" in result:  # 判断是否有token项，如果有且不为-1就把它添加到cookie里
            if result["token"] == "-1":
                ...
                response.set_cookie(
                    key="SESSION", value=str(result["token"]), max_age=14 * 24 * 3600
                )  # 添加cookie
        return response
